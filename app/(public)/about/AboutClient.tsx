...
import { formatStat, type PlatformStats } from "@/lib/stats/platform-stats";

const valuesFor = (stats: PlatformStats) => [
  {
    icon: (
    ...
    title: "Global Reach",
    desc: `Connecting workers and businesses across ${formatStat(stats.resorts)} resorts in ${formatStat(stats.countries)} countries worldwide.`,
  },
  {
    icon: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    desc: "Verified businesses carry a badge, and any we have not checked yet say so plainly on their profile. You always know who you are dealing with.",
  },
  {
    icon: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    desc: "One profile, applications you can track, and built-in video interviews when you get that far.",
  },
  {
    icon: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function AboutClient({ stats }: { stats: PlatformStats }) {
  useScrollAnimation();
        ...
        <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
          ...
          <div className="animate-on-scroll-left relative">
            ...
            <div className="absolute -bottom-6 -right-6 rounded-xl bg-white p-5 shadow-xl animate-float">
              <p className="text-3xl font-extrabold text-primary">{formatStat(stats.resorts)}</p>

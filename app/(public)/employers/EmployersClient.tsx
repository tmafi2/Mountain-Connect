...
interface EmployersClientProps {
  ...
  initialResorts: { id: string; name: string }[];
  initialLegacyResortMap: Record<string, string>;
}

export default function EmployersClient({
  initialBusinesses,
  initialResorts,
  initialLegacyResortMap,
}: EmployersClientProps) {
  const [businesses] = useState<Business[]>(initialBusinesses);
  const [allResorts] = useState(initialResorts);
  const [legacyResortMap] = useState(initialLegacyResortMap);

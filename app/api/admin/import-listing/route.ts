...
export async function POST(request: Request) {
  try {
    ...
    // Resolve the business's nearby_town_id. Prefer the explicit value
    // the admin picked in the form; fall back to inferring from the
    // location text (exact match against nearby_towns.name) so a free-
    // text "Jindabyne" or "Jindabyne, NSW" still stamps the FK.
    const businessTownId =
      nearbyTownId ||
    ...
    if (existing) {
      ...
      // Reuse the existing unclaimed shell. If the shell was missing a
      // nearby_town_id but we just resolved one, stamp it now —
      ...
      if (!existing.nearby_town_id && businessTownId) {
        await admin
          .from("business_profiles")
          .update({ nearby_town_id: businessTownId })
          .eq("id", existing.id)

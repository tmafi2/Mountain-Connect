  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] items-center justify-center rounded-2xl border border-accent/40 bg-white">
      <div className="text-center px-6">
        ...
        <h2 className="text-xl font-bold text-primary">Messages Coming Soon</h2>
        <p className="mt-2 text-sm text-foreground/60 max-w-md">
          We&apos;re putting the finishing touches on our messaging system. You&apos;ll be able to chat directly with businesses and employers very soon.
        </p>
      </div>
    </div>
  );
    ...
    if (!participations || participations.length === 0) {
      ...
      if (selectConvId) {
        ...
        const { data: retry } = await supabase
          ...
          .select("conversation_id, role")
          .eq("user_id", user.id);
        if (!retry || retry.length === 0) { setLoading(false); return; }
        // Continue with retry data
        return loadConversationsFromParticipations(supabase, user.id, retry, selectConvId);
      }
      setLoading(false);
      return;
    }

    return loadConversationsFromParticipations(supabase, user.id, participations, selectConvId);
  }, []);

  const loadConversationsFromParticipations = async (
    supabase: ReturnType<typeof createClient>,
    userId: string,
    participations: { conversation_id: string; role: string }[],
    selectConvId?: string,
  ) => {
      ...
      {showNewConvModal && currentUserId && (
        <NewConversationModal
          ...
          onConversationCreated={async (data: ConversationCreatedData) => {
            ...
            setLoading(true);
            // Wait for admin writes to propagate, then reload conversation list

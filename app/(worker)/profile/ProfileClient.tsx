...
const COUNTRY_FLAGS: Record<string, string> = {
  ...
  "United States": "\u{1F1FA}\u{1F1F8}", "Country not listed": "\u{1F3D4}\u{FE0F}",
};
...
export default function ProfileClient({ initialProfile, initialAvatarUrl }: ProfileClientProps) {
  const [profile] = useState<WorkerProfile | null>(initialProfile);
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
              ...
              {profile?.nationality && (
                <span className="flex items-center gap-1 rounded-full bg-accent/20 px-2.5 py-1 text-foreground/60">
                  {COUNTRY_FLAGS[profile.nationality] || "\u{1F30D}"} {profile.nationality}
                </span>
              )}
      ...
      {profile && (
        <div className="mt-6 space-y-4">
                  ...
                  {(profile.work_authorizations as { country: string; visa_status: string; visa_expiry: string | null }[]).map((wa) => (
                    <div key={wa.country} className="flex flex-wrap items-center gap-2 rounded-lg bg-green-50 px-3 py-2">
                      <span className="text-sm font-medium text-green-800">
                        {COUNTRY_FLAGS[wa.country] || "\u{1F30D}"} {wa.country}
                      </span>
                      {wa.visa_status && (
                        <>
          ...
          {profile.references && profile.references.length > 0 && (
            ...
            >
              <div className="space-y-3">
                {profile.references.map((ref: { id: string; name: string; relationship: string; type: string; company: string | null; job_title: string | null; email: string; phone: string | null; notes: string | null; location?: string }, i: number) => (

...
function Section({ icon, iconBg, title, children }: {
  ...
  return (
    <div className="rounded-2xl border border-accent/50 bg-white/70 p-6 shadow-sm backdrop-blur-sm">
...
export default function BusinessWorkerProfilePage() {
  ...
  if (!profile) {
    return (
      <div className="mx-auto max-w-3xl py-10 text-center">
        <p className="text-foreground/50">Worker profile not found.</p>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-primary-light to-primary px-6 pb-8 pt-8">
        ...
        <div className="relative">
          ...
          <p className="mt-1 text-sm text-white/60">
            Viewing {name || "worker"}&apos;s full profile
          </p>
        </div>
      </div>

      {/* Profile header card */}
      <div className="mt-6 rounded-2xl border border-accent/50 bg-white/70 p-8 shadow-sm backdrop-blur-sm">
        <div className="flex items-center gap-5">
          {/* Profile picture */}
          <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full bg-secondary/20 ring-4 ring-secondary/20">
            {avatarUrl && avatarUrl.startsWith("flag:") ? (
              <span className="text-5xl">{avatarUrl.replace("flag:", "")}</span>
            ) : avatarUrl ? (
              <Image src={avatarUrl} alt="Profile" fill className="object-cover" style={{ position: "relative" }} />
      ...
      <div className="mt-6 space-y-4">
        ...
        {profile.references && profile.references.length > 0 && (
          ...
          >
            <div className="space-y-3">
              {profile.references.map((ref: any, i: number) => (
        ...
        {(profile.cv_url || profile.cover_letter_url) && (
          ...
          >
            <div className="flex flex-wrap gap-3">
              {profile.cv_url && (
                <ResumeViewer resumePath={profile.cv_url} fileName="Resume / CV" variant="button" />
              )}
              {profile.cover_letter_url && (
                <ResumeViewer resumePath={profile.cover_letter_url} fileName="Cover Letter" variant="button" />
              )}
            </div>
          </Section>
        )}

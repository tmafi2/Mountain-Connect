...
export function areaJobsUpdateEmail({
  ...
  return {
    subject: `New jobs in ${areaName} — apply early ⛷️`,
    text: [
      `Hey ${greetingName},`,
      "",
      `Fresh listings have just gone up in ${areaName} — here are ${jobs.length} of the newest jobs employers are hiring for right now:`,
      "",
      ...jobs.map(
        (j) =>
      ...
      "",
      `Browse all ${areaName} jobs: ${browseUrl}`,
      "",
      "Cheers,",
      "Tyler",
      "Mountain Connects",
    ...
    <tr><td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
          ...
          <tr><td style="padding:36px 32px 24px;">
              <p style="margin:0 0 18px;color:#4e5d6c;font-size:15px;line-height:1.7;">Hi ${greetingName},</p>
              <p style="margin:0 0 24px;color:#4e5d6c;font-size:15px;line-height:1.7;">Fresh listings have just gone up in ${areaName} — here are ${jobs.length} of the newest jobs employers are hiring for right now:</p>
              <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 8px;">
                ${jobRows}
              </table>
              <table cellpadding="0" cellspacing="0" style="margin:24px auto 8px;" align="center">
                <tr><td bgcolor="#3b82f6" style="background-color:#3b82f6;background-image:linear-gradient(135deg,#22d3ee,#3b82f6);border-radius:10px;padding:15px 36px;text-align:center;">
                    <a href="${browseUrl}" style="color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;">Browse all ${areaName} jobs →</a>
                </td></tr>
              </table>
              <p style="margin:24px 0 0;color:#8899a6;font-size:12px;text-align:center;">
                Not interested in updates like these? Just reply and I'll take you off the list.
              </p>
          </td></tr>

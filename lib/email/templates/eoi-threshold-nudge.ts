interface EoiThresholdNudgeEmailProps {
  businessName: string;
  jobTitle: string;
  eoiCount: number;
  claimUrl: string;
...
export function eoiThresholdNudgeEmail({ businessName, jobTitle, eoiCount, claimUrl }: EoiThresholdNudgeEmailProps) {
  return {
    subject: `${eoiCount} workers want your ${jobTitle} job`,
    html: `
<!DOCTYPE html>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#0a1e33 0%,#0f2942 40%,#1a3a5c 100%);padding:44px 32px 36px;text-align:center;">
            ...
            <h1 style="margin:0 0 6px;color:#ffffff;font-size:26px;font-weight:800;letter-spacing:-0.5px;">${eoiCount} workers are interested</h1>
        ...
        <tr>
          <td style="padding:36px 32px 24px;">
            ...
            <p style="margin:0 0 16px;color:#4e5d6c;font-size:15px;line-height:1.7;">
              Quick heads-up — <strong style="color:#0a1e33;">${eoiCount} job seekers</strong> have already expressed interest in your <strong>${jobTitle}</strong> listing on Mountain Connects.
            </p>
            <p style="margin:0 0 24px;color:#4e5d6c;font-size:15px;line-height:1.7;">
              They're ready to hear from you. Claim the listing to see their details, message them directly, and start interviewing — all in one place.
            </p>
            ...
            <table cellpadding="0" cellspacing="0" style="margin:0 auto 28px;" align="center">
              <tr>
                <td style="background:linear-gradient(135deg,#22d3ee,#3b82f6);border-radius:10px;padding:15px 36px;text-align:center;">
                  <a href="${claimUrl}" style="color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;">See interested candidates →</a>
                </td>
              </tr>
            </table>
